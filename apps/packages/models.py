...
class Package(BaseModel):
    """
    Main package model that combines multiple services
    """
    PACKAGE_TYPES = [
        ('hajj', 'Hajj'),
        ('umrah', 'Umrah'),
    ...
    booking_deadline = models.DateField(
        help_text="Last date for booking this package"
    )
    ...
    def final_price(self):
        """Return final price (discounted if available)"""
...
class PackageItinerary(BaseModel):
    """
    Day-wise itinerary for the package
    """
    package = models.ForeignKey(
        Package, 
        on_delete=models.CASCADE, 
...
class PackageAvailability(BaseModel):
    """
    Date-wise availability for packages (especially for air tickets)
    """
    package = models.ForeignKey(
        Package, 
        on_delete=models.CASCADE, 
    ...
    color_code = models.CharField(
        max_length=7, 
        default='#28a745',
        help_text="Color code for calendar display"
    )
